...
見分けやすくする。issue #6 の対応。P3 が拾う前にスタイルごとに 1 回走る
(イタリックは同ウェイトのソースを共有しつつ、オーバーライドだけ分けられる)。
ログは `build/sfms/dakuten.<style>.log`。
...
- 溶接で濁点が欠ける字 — 片方の点が本体輪郭の中にある（グ ゴ ゾ ダ ブ）、点が本体と接触して欠けている（Bold に多い。本体接触と凸性欠損で検出）— は、二つの点が常に一緒に動き一緒に削られるよう修復する。欠けた点はその字自身の無傷の点のクローンで置き換え、無傷の点が残っていない場合だけ同スクリプトのきれいなマークで丸ごと再構築する
...
グローバル値の上に字ごとの調整を重ねられる。`uv run scripts/dakuten_tuner.py`
を実行すると `http://localhost:8765` にビジュアルエディタが立つ。
濁音かなの一覧から字を選び、マークの倍率・回転・削りギャップ (全体 + 左右上下)
はスライダー、位置はドラッグ、除外はチェックボックスで調整する。
スタイルタブは Regular / Bold / Italic / BoldItalic の 4 つで、
どのスタイルも Regular の値を継承し、他タブでの変更はそのスタイルだけの
差分として保存される。保存先は `scripts/dakuten_overrides.json`
(字 → `scale` / `aspect` / `spread` / `halo` / `rot` / `halo_pad` / `dx` / `dy` /
`skip_ink` / `exclude` + 任意の `bold` / `italic` / `bolditalic` サブオブジェクト、
font units) で、次のビルドから反映される。プレビューは本体の上にハロを
紙色で塗る方式で、ビルドと同じコードパスを通るためブーリアン削りと
見た目が完全に一致する。
