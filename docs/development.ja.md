...
- `KM_DAKUTEN_SCALE` / `KM_HANDAKUTEN_SCALE` がマークの倍率 (1.3 / 1.25)、`KM_DAKUTEN_HALO` / `KM_HANDAKUTEN_HALO` が削りギャップ (0.40 / 0.30)、`KM_DAKUTEN_SKIP_INK=0` で削りを無効化、`KM_DAKUTEN_EXCLUDE` が対象外の字 (既定 ゞヾヷヸヹヺ)
